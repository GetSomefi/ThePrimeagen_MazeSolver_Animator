//compile: npx tsc .\RecursionMazeSolver.ts  
//run compiled project: node RecursionMazeSolver.js
type Point = {
    x:number,
    y:number
}
...
function recursiveForLoop(i:number):void {
    setTimeout(function() {
        console.clear();
        ...
        if (i<mazeAnimation.length-1) recursiveForLoop(++i);   //  decrement i and call myLoop again if i > 0
    }, 100)
};
